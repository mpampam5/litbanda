  ...
  <div class='box-header with-border'>
    <h3 class='box-title'>Modul <?=ucfirst($layout_title)?></h3>
      <div class='box-tools pull-right'>
        <a href='<?=site_url(config_item("cpanel")."kategori/tambah")?>' id='tambah' class='btn btn-success btn-sm'> Tambah</a>
      </div>
  </div>
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                ...
                var t = $("#mytable").dataTable({
                    ...
                    processing: true,
                    serverSide: true,
                    ajax: {"url": "<?=base_url(config_item("cpanel"))?>/kategori/json", "type": "POST"},
                    columns: [
                        {"data": "kategori"},
                        {
                            "data" : "action",
           ...
           $(document).on('click', '#tambah', function(e){
               ...
               $('#ModalHeader').html('Tambah <?=ucfirst($layout_title)?>');
           ...
           //
            // MODAL EDIT
             $(document).on('click', '#edit', function(e){
                 ...
                 $('.modal-dialog').removeClass('modal-sm');
                 $('.modal-dialog').addClass('modal-md');
                 $('#ModalHeader').html('Edit <?=ucfirst($layout_title)?>');
                 $('#ModalContent').load($(this).attr('href'));
                 $('#ModalGue').modal('show');
               });
           //
