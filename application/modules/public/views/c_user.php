...
WHERE a.turunan=0  
 ...
 INNER JOIN `dd_kuantitas` g ON `g`.`id_dd_kuantitas`=`d`.`satuan_kuantitas` 
    ...
    public function turunan($id, $id_opmt_bulanan, $id_turunan) {
        ...
        $data['dt_kuantitas'] = $this->db->order_by('satuan_kuantitas','ASC')->get('dd_kuantitas')->result_array();
...
GROUP BY a.id_opmt_target_skp")->result_array();
        ...
        $x['spesimen'] = $this->db->query('SELECT * FROM dd_spesimen')->result_array();
...
INNER JOIN opmt_target_skp b on a.id_opmt_tahunan_skp=b.id_opmt_tahunan_skp AND a.id_dd_user=(select atasan_langsung from dd_user where id_dd_user={$id_user})")->result_array();
        $data['dt_kuantitas'] = $this->db->order_by('satuan_kuantitas','ASC')->get('dd_kuantitas')->result_array();
        $periode = $this->db->query("SELECT * FROM opmt_tahunan_skp WHERE id_opmt_tahunan_skp='" . $id . "'")->row_array();
	$tahun=date('Y',strtotime($periode['akhir_periode_skp']));
        $data['periode'] = $periode;
...
where kodeunit='{$data_user['kodeunit']}' AND a.tahun={$tahun}")->result_array();
...
WHERE a.id_dd_user={$id_user} AND year(a.akhir_periode_skp)='{$data_bulanan['tahun']}'")->result_array();
        $data['dt_kuantitas'] = $this->db->order_by('satuan_kuantitas','ASC')->get('dd_kuantitas')->result_array();
        $data['periode'] = $this->db->query("SELECT * FROM opmt_tahunan_skp WHERE id_opmt_tahunan_skp='" . $id . "'")->row_array();
...
WHERE a.id_dd_user={$id_user}")->result_array();
    ...
    public function aksi_bulanan_skp() {
        ...
        $cek = $this->db->from("opmt_bulanan_skp")->where("bulan", $p->bulan)->where("tahun", $p->tahun)->where("id_dd_user", $this->session->userdata('id_user'))->count_all_results();
        if ($cek > 0) {
            $a['status'] = 0;
            $a['ket'] = 'Tahun ' . $p->tahun . ' Bulan ' . $p->bulan . ' Sudah Ada !!!';
            $this->j($a);
            die();
        }
